...
int get_label_camera_planes(ecs *world, entity player, char buffer[], int buffer_size, int buffer_index) {
    ...
    for (int i = 0; i < CameraPlanes_length; i++) {
        buffer_index += snprintf(buffer + buffer_index, buffer_size - buffer_index, "%i [%.1fx%.1fx%.1f] [%.1f]\n", (i + 1), planes->value[i].normal.x, planes->value[i].normal.y,  planes->value[i].normal.z, planes->value[i].distance);
    }
    // const float6 bounds = zox_getv(camera, Position3DBounds)
...
int get_label_camera_frustum(ecs *world, entity player, char buffer[], int buffer_size, int buffer_index) {
    ...
    for (int i = 0; i < FrustumCorners_length; i++) {
