    ...
    for (int i = 0; i < it->count; i++) {
        ...
        while (zox_children_next(it2)) {
            for (int j = 0; j < it2.count; j++) {
                ...
                entity prefab = zox_get_prefab(world, e2);
                if (prefab == quest) {
                    has_quest = 1;
                }
                quests_count++;
        ...
        while (1) {
            ...
            if (has_quest && zox_has(possible_node, NodeTrue)) {
                next = possible_node;
                break;
            } else if (!has_quest && zox_has(possible_node, NodeFalse)) {
                next = possible_node;
                break;
            }
            node_index++;
        ...
        delay_event(world, &delayed_node_end, e, 0.25);
        // zox_setv(e, NodeEnd, zox_dirty_trigger);
        if (dbg_log) {
            zox_log("[zox_event_has_quest] %s: Nodes [%s -> %s] - Character [%s] - Has Quest [%i]",
                zox_getn(e),
                zox_getn(node),
                zox_getn(next),
