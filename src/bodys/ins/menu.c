...
entity spawn_player_menu_body(ecs* world, entity player) {
    ...
    // Spawn our window
    entity e = spawn_datagrid_slots2(world, prefab_window, prefab_frame_selectable, prefab_icon, prefab_label, label_font_size, canvas, character, cells_size, "Body", frame_equip_fill, frame_equip_outline, position_anchor, position, frame_id, parts->data, parts->size);
    zox_add_tag(e, MenuBody);
    dispose_entity_array_d(parts);
    return e;
}
