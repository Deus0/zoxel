...
static inline void fetch_nearby_chunks(
    ecs* world,
    entity e,
    ...
    spinlock** locks)
{
    for (byte i = 0; i < 27; i++) {
        ...
        lights[i] = zox_get(chunk, LightNode);
        LightLock* lock = zox_mut(chunk, LightLock);
        locks[i] = &(lock->value);
    }
}

// NOTE: Grabs neighbors and corners
/*void fetch_neightbor_lights(
    ecs* world,
