...
static inline byte flood_light(const VoxelNode* root_vnode, LightNode* root_lnode, const VoxelNode* n_root_vnodes[6], const LightNode* n_root_lnodes[6], LightQueue* n_queues[6], byte depth,
    byte3 positionl,
    byte light,
    byte distance,
    byte min_light,
...
zox_sys2(LightFloodSystem) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        while (light_queue->count) {
            ...
            if (voxel && solidity[voxel - 1]) {
                if (dbg_log) {
                    zox_log("[%s]: Light Flood Canceled at [%ix%ix%i] l[%i] q [%i]", zox_get_name(it->entities[i]), update.pos.x, update.pos.y, update.pos.z, update.light,  light_queue->count);
                }
                continue;
            }
