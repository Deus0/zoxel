...
int read_vox(const char* filename, vox_file *vox) {
    ...
    if (!file) {
        zox_loge("[vox] failed to open file [%s]", filename);
        return EXIT_FAILURE;
    }
    long file_size = get_file_size(file);
    if (file_size <= 0) {
        zox_loge("[vox] invalid file size for [%s]", filename);
        fclose(file);
        return EXIT_FAILURE;
    }
    byte *file_buffer = malloc(file_size);
    if (!file_buffer || fread(file_buffer, file_size, 1, file) != 1) {
        zox_loge("[vox] failed to read file [%s]", filename);
        free(file_buffer);
        fclose(file);
        return EXIT_FAILURE;
    }
    ...
    if (strncmp(vox->header.magic, "VOX ", 4) != 0) {
        zox_loge("[vox] invalid magic value");
        free(file_buffer);
        return EXIT_FAILURE;
    }
    ...
    if (ptr + 4 <= file_buffer + file_size && !strncmp((char*)ptr, "PACK", 4)) {
        if (ptr + sizeof(vox_file_pack) > file_buffer + file_size) {
            zox_loge("[vox] file too small for pack chunk");
            free(file_buffer);
            return EXIT_FAILURE;
        }
    ...
    if (vox->pack.model_nums <= 0) {
        zox_loge("[vox] invalid model_nums: %d", vox->pack.model_nums);
        free(file_buffer);
        return EXIT_FAILURE;
    }
    if (dbg_log) zox_log(" + [vox] model_nums: %d", vox->pack.model_nums);
    // Allocate chunks and parse in single pass
    vox->chunks = malloc(vox->pack.model_nums * sizeof(vox_file_chunk_child));
    if (!vox->chunks) {
        zox_loge("[vox] failed to allocate chunks");
        free(file_buffer);
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < vox->pack.model_nums; i++) {
        if (ptr + sizeof(vox_file_size) + 12 > file_buffer + file_size) {
            zox_loge("[vox] file too small for chunk %d", i);
            free(vox->chunks);
            free(file_buffer);
            return EXIT_FAILURE;
        }
        ...
        if (voxels_length <= 0) {
            zox_loge("[vox] invalid voxel length for chunk %d", i);
            free(vox->chunks);
            free(file_buffer);
            return EXIT_FAILURE;
        }
        ...
        if (num_voxels < 0 || ptr + num_voxels * 4 > file_buffer + file_size) {
            zox_loge("[vox] invalid num_voxels for chunk %d: %d", i, num_voxels);
            free(vox->chunks);
            free(file_buffer);
            return EXIT_FAILURE;
        }
        if (!voxel_ptr) {
            ...
            if (!voxel_ptr) {
                zox_loge("[vox] failed to allocate voxel buffer");
                free(vox->chunks);
                free(file_buffer);
                return EXIT_FAILURE;
            }
        ...
        if (dbg_log) zox_log("[vox] chunk %d num_voxels: %d", i, num_voxels);
        for (int j = 0; j < num_voxels; j++) {
            ...
            if (array_index < 0 || array_index >= voxels_length) {
                zox_loge("[vox] invalid array index for chunk %d voxel %d: %d", i, j, array_index);
    ...
    if (dbg_log) zox_log(" + [vox] chunk parse time: %.2f ms", chunk_parse_time);

    // Parse palette
    start = get_time_ms();
    if (dbg_log) zox_log(" + [vox] palette ptr offset: %ld, file_size: %ld", ptr - file_buffer, file_size);
    if (ptr + 4 <= file_buffer + file_size && !strncmp((char*)ptr, "RGBA", 4)) {
        if (dbg_log) zox_log(" + [vox] found RGBA chunk");
        if (ptr + 12 > file_buffer + file_size) {
            zox_loge("[vox] file too small for palette header");
            free(vox->chunks[0].xyzi.voxels);
            free(vox->chunks);
            free(file_buffer);
            return EXIT_FAILURE;
        ...
        if (dbg_log) zox_log(" + [vox] palette colors_length: %d", colors_length);
        if (colors_length <= 0 || ptr + sizeof(color) * colors_length > file_buffer + file_size) {
            zox_loge("[vox] invalid palette colors_length: %d", colors_length);
            free(vox->chunks[0].xyzi.voxels);
            free(vox->chunks);
            free(file_buffer);
            return EXIT_FAILURE;
        ...
        if (!vox->palette.values || !vox->palette.values_rgb) {
            zox_loge("[vox] failed to allocate palette");
        ...
        if (dbg_log) zox_log(" + [vox] using default palette");
        int colors_length = 256;
        vox->palette.values = malloc(sizeof(color) * colors_length);
        vox->palette.values_rgb = malloc(sizeof(color_rgb) * colors_length);
        if (!vox->palette.values || !vox->palette.values_rgb) {
            zox_loge("[vox] failed to allocate default palette");
            free(vox->palette.values);
            free(vox->palette.values_rgb);
            free(vox->chunks[0].xyzi.voxels);
            free(vox->chunks);
    ...
    if (dbg_log) zox_log(" + [vox] palette parse time: %.2f ms", palette_parse_time);
    free(file_buffer);
    double total_time = get_time_ms() - start_total;
    if (dbg_log) zox_log(" + [vox] total time: %.2f ms, unaccounted: %.2f ms", total_time, total_time - section_sum);
    return EXIT_SUCCESS;
}
