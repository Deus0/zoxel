    ...
    SpawnBlock data)
{
    ...
    if (data.prefab_world_block) {
        zox_setv(e, BlockPrefabLink, data.prefab_world_block);
    }
    if (data.tag) {
    ...
    if (data.model_type) {
        zox_setv(e, BlockModel, data.model_type);
    }
    if (data.vox_offset) {
        zox_setv(e, BlockVoxOffset, 1);
    }
    // if not bake
    ...
    if (data.model) {
        zox_setv(e, ModelLink, data.model);
    }
    ...
    for (byte i = 0; i < 6; i++) {
        ...
        zox_setv(e2, ModelLink, data.model);
