...
zox_sys2(TilemapUVSystem) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        while (zox_query_next(texture_it2)) {
        ...
        for (int y = 0; y < map_size->value.y; y++) {
            for (int x = 0; x < map_size->value.x; x++) {
                ...
                // Now
                byte face_index = zox_getv(texture, ChildIndex);
                /*byte face_index = 0;
                ...
                byte repeated = zox_has(block, SingleTextureBlock);
        ...
        if (dbg_log) {
            zox_log("Tilemap UVs Generated: Unit Size [%ix%i] Blocks [%i] Textures [X] MapSize [%ix%i]",
                unit_size.x,
                unit_size.y,
                blocks->length,
                map_size->value.x,
