void fetch_children_blocks(ecs *world, BlockLinks* blocks, entity e, byte dbg_log) {
    if (zox_has(e, Block)) {
    ...
    uint length = zox_get_children(world, e, children, zox_children_capacity);
    for (uint k = 0; k < length; k++) {
        entity e2 = children[k];
        fetch_children_blocks(world, blocks, e2, dbg_log);
    }
}
...
zox_sys2(RealmBlocksDirtySystem) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        if (dbg_log) {
            zox_log("Realm Blocks Dirty [%s] Blocks [%i]", zox_get_name(e), blocks->length);
        }
