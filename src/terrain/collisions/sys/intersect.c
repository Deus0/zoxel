...
zox_sys2(TerrainIntersectSystem) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        float3 block_size = float3_single(terrain_scale * 0.5f);
        ...
        int3 lower_bounds_block_position = real_position_to_block_position(lower_bounds, terrain_scale);
        int3 upper_bounds_block_position = real_position_to_block_position(upper_bounds, terrain_scale);
        float3 correction = float3_zero;
        // we just need add + terrain_scale and final one too
        float add = terrain_scale;
        byte hit_ground = 0;
        byte hit_axis_x = 0;
        byte hit_axis_y = 0;
        byte hit_axis_z = 0;
        ...
        for (point.x = lower_bounds.x; point.x <= upper_bounds.x; point.x = point.x == upper_bounds.x ? upper_bounds.x + 1 : clampf(point.x + add, lower_bounds.x, upper_bounds.x)) {
            for (point.y = lower_bounds.y; point.y <= upper_bounds.y; point.y = point.y == upper_bounds.y ? upper_bounds.y + 1 : clampf(point.y + add, lower_bounds.y, upper_bounds.y)) {
                for (point.z = lower_bounds.z; point.z <= upper_bounds.z; point.z = point.z == upper_bounds.z ? upper_bounds.z + 1 : clampf(point.z + add, lower_bounds.z, upper_bounds.z)) {
                    int3 block_position = real_position_to_block_position(point, terrain_scale);
                    if (int3_equals(block_position, last_block_position)) {
                        continue;
                    }
                    last_block_position = block_position;
                    float3 block_positionf = block_position_to_real_position(block_position, terrain_scale);
                    block_positionf = float3_add(block_positionf, float3_single(terrain_scale / 2));
                    ...
                    // now spawn lines
                    if (dbg_lines) {
                        // NOTE: This is for all blocks we intersect with
                    ...
                    if (dbg_lines) {
                        ...
                        spawn_line3_alpha(world, point, block_position_face, 4, 0.01, color_black);
                        spawn_cube_lines_rgba(world, block_position_face, float3_single(0.03f), 2, fill, 0.01);
                    }
                }
            }
        }
