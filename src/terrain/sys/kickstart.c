...
zox_sys2(FirstTerrainTunkSystem) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        zox_mut_begin(terrain->value, Chunk2Links, chunks2);
        if (int2_hashmap_has(chunks2->value, position->value)) {
            continue;
        }
        ...
        int2_hashmap_add(chunks2->value, position->value, tunk);
        zox_mut_end(terrain->value, Chunk2Links);
    }
