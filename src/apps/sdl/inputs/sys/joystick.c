...
zox_sys2(SdlJoystickFetchSystem) {
    byte dbg_log = 0;
    byte dbg_log_all = 0;
    zox_sys_world();
    zox_sys_begin();
    zox_sys_in(SdlJoystick);
    for (int i = 0; i < it->count; i++) {
        zox_sys_e();
        zox_sys_i(SdlJoystick, joystick);
        if (!sdl_gamepad_handle_disconnect(joystick->value)) {
            if (dbg_log) {
        ...
        for (uint j = 0; j < children_length; j++) {
            ...
            extract_sdl_gamepad_button(world, e2, joystick->value, dbg_log);
        }
        // Debug new gamepads!
        if (dbg_log_all) {
            // Axes
            int axis_count = SDL_JoystickNumAxes(joystick->value);
            for (int j = 0; j < axis_count - 2; j++) {
                Sint16 value = SDL_JoystickGetAxis(joystick->value, j);
            ...
            // Buttons
            int button_count = SDL_JoystickNumButtons(joystick->value);
            for (int j = 0; j < button_count; j++) {
                if (SDL_JoystickGetButton(joystick->value, j)) {
                    zox_log("Button %i", j);
                }
            }
            // Hats (D-pad on many devices)
            int hat_count = SDL_JoystickNumHats(joystick->value);
            for (int j = 0; j < hat_count; j++) {
                Uint8 hat = SDL_JoystickGetHat(joystick->value, j);
