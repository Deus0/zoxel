...
SDL_Finger* find_finger_unused(ecs *world, const Children *zevices) {
...
void sdl_extract_touchscreen(ecs *world, const Children *children, int2 touchscreen_size) {
    touch_fingers_count = 0;
    touch_devices_count = SDL_GetNumTouchDevices();
    for (int i = 0; i < children->length; i++) {
        entity zevice = children->value[i];
        if (!zox_valid(zevice) || !zox_has(zevice, Finger)) {
            continue;
        }
        int zevice_id = zox_get_value(zevice, ID)
        if (zevice_id) {
