...
void initialize_sdl_gamepads(ecs *world, entity app) {
    ...
    if (dbg_log) {
        zox_log("Startup Gamepads: [%d]", joysticks_count);
    }
    for (int i = 0; i < joysticks_count; i++) {
        if (using_sdl_gamecontrollers) {
            SDL_JoystickID id = zox_get_gamepad(i);
            if (zox_is_gamepad(id)) {
                zox_sdl_gamepad* controller = zox_sdl_gamepad_open(id);
                if (!controller) {
                    zox_loge("Joystick Error: %s", SDL_GetError());
                    continue;
                }
                if (dbg_log) {
...
static inline byte process_input_button(byte old_byte, byte raw_value) {
