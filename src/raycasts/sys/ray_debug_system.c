...
zox_sys2(RayDebugSystem) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        spawn_line3t(world, origin->value, float3_add(origin->value, end_point), debug_color);
        spawn_line3t(world, origin->value, float3_add(origin->value, end_point2), debug_color2);
    }
