entity spawn_statbar2(ecs* world, entity parent, entity stat, int2 position, int2 size, float2 anchor, color_rgb fill, byte font_size) {
    if (!zox_valid(stat)) {
    ...
    entity2 e2 = spawn_elementbar2(world, prefab_statbar2D, parent, position, size, anchor, font_size, padding, back_color, front_color);
