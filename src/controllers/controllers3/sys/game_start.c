...
// Trigger our streamers to start loading terrain chunks
void delayed_start_streamer(ecs *world, entity player) {
    ...
    spawn_first_chunk(world, realm, terrain, position);
...
zox_sys2(PlayerGame3StartSystem) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        zox_log("Spawning player character at [%fx%fx%f]", position.x, position.y, position.z);
