...
void get_home_directory(
    ...
    if (home_directory) {
        snprintf(path, size, "%s\\AppData\\Local\\", home_directory);
    } else {
        zox_log_error("home_directory null [get_save_directory]")
    }
    ...
    snprintf(path, size, "%s/", path_cstr);
    (*env)->ReleaseStringUTFChars(env, path_string, path_cstr);
#else
    const char *home_directory = getenv("HOME");
    if (home_directory) {
        snprintf(path, size, "%s/.local/share/", home_directory);
    } else {
        zox_log_error("home_directory null [get_save_directory]")
    }
#endif
...
byte create_new_save_directory(const char *game) {
    ...
    return create_new_directory(path);
}
