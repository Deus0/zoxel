...
entity3 spawn_bar3(ecs *world, float3 position, float scale, int2 size, byte font_size, color fill, color outline,  color fill2, color outline2, color font_fill, color font_outline, entity owner, float trail_offset) {
    ...
    output.x = e;
    {
        entity panel = spawn_uic(world, prefab_frame3, e, float2_centre, int2_zero, size, size, fill, outline);
        output.y = panel;
        zox_setv(panel, Layer2D, 0);
        zox_add_tag(panel, Elementbar);
        ...
        zox_set_unique_name(panel, "bar_panel");
        {
            ...
            entity e2 = spawn_uic(world, prefab_frame3, panel, float2_centre, front_position, front_size, front_size, fill2, outline2);
            zox_add_tag(e2, BarFront);
            zox_setv(e2, Layer2D, 1);
            zox_set_unique_name(e2, "frontbar");
        }
        {
            entity e2 = spawn_text(world, prefab_text3, panel, int2_zero, float2_centre, font_size, zox_alignment_centre, text_padding, "Health", font_fill, font_outline);
            output.z = e2;
