...
zox_sys2(ElementActiveLinkSystem) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        if (!zox_valid(parent->value) || !zox_has(parent->value, ActiveLink)) {
            continue;
        }

        zox_mut_begin(parent->value, ActiveLink, active);
        ...
        if (zox_valid(active->value) && zox_has(parent->value, ActiveSingle)) {
        ...
        active->value = e;
        zox_modified(parent->value, ActiveLink);
    }
