entity spawn_header(
    ...
    SpawnZext zext_spawn_data = {
        ...
        .zext = {
            .text = label,
            .font_size = font_size,
            .font_resolution = font_size, // header_font_resolution,
...
entity spawn_header2(ecs *world, SpawnHeader *data) {
    int string_length = strlen(data->zext.text);
    int2 zext_position = (int2) {
    ...
    byte2 padding = (byte2) { (int) (data->zext.font_size * 0.3f), (int) (data->zext.font_size * 0.3f) };

    byte zext_layer = data->element.layer + 1;
    byte button_layer = data->element.layer + 2;
    ...
    entity header_zext = spawn_zext(world, &zextSpawnData);
    ...
    if (data->header.is_close_button) {
        ...
        entity b = spawn_close_button(world, e, data->canvas.e, close_button_position, data->zext.font_size, padding, button_layer, (ClickEvent) { NULL });
        add_to_Children(&children, b);
    }
