zox_sys2(TextParentBackgroundSystem) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        if (!zox_valid(parent) || !zox_has(parent, LayoutSize) || !zox_has(parent, TextResizer)) {
            continue;
        }
        ...
        if (!int2_equals(old_size, new_size)) {
            zox_set(parent, LayoutSize, { new_size });
            zox_set(parent, LayoutSizeDirty, { zox_dirty_trigger });
        }
    }
