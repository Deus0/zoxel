...
byte save_font_style(ecs *world, const entity e, char *resources_directory, char *filename) {
    ...
    zox_log(" + saving font style entity [%s]", zox_get_name(e))
    zox_log("   - to zox file [%s]", filename)
    zox_log("   - directory [%s]", directory)
    zox_log("   - full path [%s]", path)
#endif
    free(directory);
    free(directory2);
    FILE *file = fopen(path, "wb");
    if (file == NULL) {
        zox_log(" > error saving [%s]", path)
...
#ifdef zox_log_font_io
        zox_log(" > font [%i] - points: [%i]", i, fontData->length)
#endif
...
#ifdef zox_log_font_io
            zox_log("   - font [%i] - points: [%i]", i, data->fonts[i].length)
            if (i == 0) {
...
entity load_font_style(ecs *world, char *filename) {
    ...
    if (filesize > 0) {
        ...
        zox_log(" ! filesize is 0 at [%s]", path)
