...
entity spawn_render_frame(ecs *world, entity parent, float2 anchor, int2 position, int2 size, entity target, float3 camera_position, float4 camera_rotation) {
    byte is_camera_filtering = 1;
    float downscale = 4;
    color background = (color) { 155, 155, 155, 0 };
    byte layer = 0; // max_layers2D - 10;
    int2 texture_size = int2_scalef(size, 1 / downscale);
...
entity spawn_render_frame_at(ecs *world, entity parent, float2 anchor, int2 position, int2 size, entity target) {
    ...
    return spawn_render_frame(world, parent, anchor, position, size, target, camera_position, camera_rotation);
}
