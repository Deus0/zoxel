entity spawn_character3(ecs *world, spawn_character3D_data data) {
    ...
    if (zox_valid(model) && zox_has(model, ModelLods)) {
        zox_geter(model, ModelLods, lods);
        entity vox_lod = lods->value[data.render_depth];
        if (zox_valid(vox_lod)) {
            model = vox_lod;
        } else {
            zox_log_error("[spawn_character3]: Invalid Vox Model Lod [%s] Depth [%i]", zox_get_name(model), data.render_depth);
            return 0;
        }
