...
zox_sys2(JumpForceSystem) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        acc->value.y += (zox_jump_power / zox_jump_time) * (dt / delta_time);
        jump->value -= dt;
        //acc->value.y += zox_jump_power / zox_jump_time;
        // jump->value -= delta_time;
        if (jump->value < 0) {
            jump->value = 0;
        }
    }
} zox_sys_end(JumpForceSystem);
