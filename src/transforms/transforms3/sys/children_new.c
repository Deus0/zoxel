void set_position_recursive_new(ecs* world, entity e, float3 pposition, float4 protation) {
...
zox_sys2(ChildrenPositionNewSystem) {
    zox_sys_world();
    zox_sys_begin();
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        for (uint j = 0; j < count; j++) {
            entity e2 = children[j];
            set_position_recursive_new(world, e2, position->value, rotation->value);
        }
    }
} zox_sys_end(ChildrenPositionNewSystem);
