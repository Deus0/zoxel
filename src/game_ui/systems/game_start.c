...
// loading resources
const double game_spawn_terrain_delay = 1;
const double game_load_player_delay = 1.1;
// fading
const double game_load_fade_transition_time = 0.5;
const double game_load_fade_time = 5;
...
void GameStartFaderSystem(iter *it) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        if (!(dirty->value == zox_dirty_active && state->value == zox_game_load_start)) {
            continue;
        }
